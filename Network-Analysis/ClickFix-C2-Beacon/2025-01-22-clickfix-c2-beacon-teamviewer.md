...
PCAP analysis from January 22, 2025. Shortly after booting and receiving a DHCP lease, machine `10.1.17.215` (user: **shutchenson**, hostname: **DESKTOP-L8C5GSJ**) established suspicious connections to external IPs over unencrypted HTTP (port 80). The user had searched for "Google Authenticator" and was redirected through a malicious Google ad to `authenticatoor.org` — a typo squatted fake download page. The goal of the analysis was to identify the attack vector, downloaded files, and post-infection malware behavior.
